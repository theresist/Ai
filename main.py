...
import requests  # For downloading from URLs
...
def handle_download_link(message):
    try:
        ...
        # Upload to Google Drive
        upload_to_drive(response.content, new_filename, 'link', message)
...
def upload_to_drive(file_content, file_name, file_type, message):
    try:
        ...
        bot.reply_to(
            message,
            f"{file_type.capitalize()} uploaded to Google Drive! File ID: {file.get('id')}"
        )
    except Exception as e:
